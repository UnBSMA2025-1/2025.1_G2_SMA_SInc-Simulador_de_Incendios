...
class ImageToMapConverter:
    def __init__(self):
        # Define tile type mappings based on your Java code
        self.tile_types = {
        ...
        # Color ranges for different terrain types (HSV)
        self.color_ranges = {
            'water': {
    ...
    def _map_clusters_to_tiles(self, cluster_centers):
        """Map cluster centers to tile types based on color characteristics"""
        mapping = {}
    ...
    def generate_environmental_properties(self, tile_type):
        ...
        wind_direction = np.random.randint(0, 8)  # Assuming 8 directions
    ...
    def convert_image_to_map(self, image_path, map_width, map_height, method='color'):
        ...
        # Generate full map data with environmental properties
        map_data = []
        for y in range(map_height):
            row = []
            for x in range(map_width):
    ...
    def visualize_map(self, tile_map, save_path=None):
        """Visualize the generated tile map"""
